...
### Turning previous script into a set of functions for Targets
###

## 2. Set of Forecast functions required for Targets pipeline

# 2.1 Data prep for each model
...
# 2.2 ARIMA model
# Required librares to be load in _targets.R file for this section:  library(stats), library(forecast)
# WIP ------ 

ARIMA_target <- function(forecast_data_prep){
  
  AE_type2_ts_prep <- forecast_data_prep
  
  AE_type2_ts_date_v <- AE_type2_ts_prep[,1]
  AE_type2_ts <- AE_type2_ts_prep[,2]
  
  # 1. ARIMA function - Turn data sets into TS object
  AE_type2_ts <- ts(AE_type2_to_ts[,1], start = c(2010, 8), end = c(2023, 11), frequency = 12)
  
  # 2. Setup our arima model using auto.arima() function from forecast package
  arima_model <- auto.arima(AE_type2_ts)
  # WIP
}
